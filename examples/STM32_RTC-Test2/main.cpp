...
int timezone = 1;      // change to your timezone
time_t tt, tt1;
tm_t mtt;
uint8_t dateread[11];
int globAlmCount = 0;
...
//const char * months[] = {"Dummy", "Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember" };
...
void loop()
{
  ...
  if (lastGlobAlmCount != globAlmCount || lastSPECAlmCount != SPECAlmCount ) {
    if (globAlmCount == 10) {        // to detachAlarmInterrupt and start creatAlarm after 10 times about 110s
      ...
      sprintf(s, "\n\nCompiled from: %s \n  on %s %s", __FILE__, __DATE__ , __TIME__);
      Serial.print(s);
      sprintf(s, "\n\nCompiled  %s \n",__TIMESTAMP__);
      Serial.println(s);      
      if(dispflag == false)
  ...
  if (tt1 != tt && dispflag == true )
  {
    ...
    Serial.print(s);
    sprintf(s, "RTC timestamp: %s, %u. %s %u, %02u:%02u:%02u Uhr.\n",
       weekdays[mtt.weekday], mtt.day, months[mtt.month], mtt.year+1970, mtt.hour, mtt.minute, mtt.second);
    Serial.print(s);
  }
}
...
https://github.com/rogerclarkmelbourne/Arduino_STM32/blob/master/STM32F1/libraries/RTClock/examples/Test_RTClock/Test_RTClock.ino
